...
randomWord wl = do
  randomIndex <- randomRIO (1,length wl -1)
...
freshPuzzle s = let (h:t) = s in Puzzle s [Just (const h t)] []
...
gameOver :: Puzzle -> IO ()
gameOver (Puzzle wordToGuess _ guessed) = if (length guessed) > 7 then
...
runGame puzzle = forever $ do
  let (Puzzle s f g ) = puzzle
  putStrLn ("Word to puzzle " ++ s)
  gameOver puzzle
  gameWin puzzle
  ...
  case guess of
    ...
    _ -> putStrLn "Your guess mus be a single character"

main :: IO ()
main = do
  hSetBuffering stdout NoBuffering
  word <- randomWord'
  putStrLn ("Word selected: " ++ word)
